...
- key_info: 只提取时效性不强的持久性信息。不要重复已有信息。宁可留空也不要凑数。区分标准：应纳入的是重要决定/共识、用户的核心偏好与原则、agent操作所需的关键资源（如文件路径、脚本位置）等跨多次对话仍有参考价值的信息；不应纳入的是某次具体的饮食/训练记录、临时性的讨论细节等日常事件，这些应仅通过 summary 记录。
