...
- "关键信息"：以 "- " 开头的条目列表。只保留对agent后续对话有实际参考价值的持久性信息。应纳入的：重要决定与共识、用户的核心偏好与原则、agent操作所需的关键资源（如文件路径、脚本位置）、里程碑状态。不应纳入的：某次具体的饮食/训练等日常事件记录、临时性的讨论细节——这些应仅出现在「最近记忆」中，不要在关键信息里重复。如果多条信息描述的是同一件事的不同方面，应合并为一条。宁可精简也不要凑数。
