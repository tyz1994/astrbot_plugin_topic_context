...
class DreamManager:
    ...
    async def organize_core(
        ...
        prompt = f"""你是一个记忆整理助手。以下是某个主题当前的核心记忆文件和所有记忆片段的摘要列表。
请重新整理 core.md，各部分要求如下：
...
- "最近记忆"：重新组织索引，条数不超过10条。排序以时效性为优先（最新的在前），重要性次之——因为"关键信息"部分已经记录了重要的历史信息，"最近记忆"的作用是追踪近期的动态和进展。不再重要的记忆片段可以省略。每条记忆末尾必须保留 (ID: xxx) 标识，ID 来自上方「所有片段摘要」中对应的 ID。
{user_instruction}
主题: {topic_name}
